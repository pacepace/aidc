...
| REL-09 | Supply-chain cooldown: a dependency version MUST NOT be adopted until it has been published for at least 14 days, so a hijacked or malicious release has time to be found and pulled before aidc installs it. For the MCP server's Python dependencies this is enforced by `exclude-newer = "14 days"` in `mcp/pyproject.toml`, recorded in `uv.lock`, and pinned by a unit test. A fix that must land sooner is an explicit exception for that one package (`exclude-newer-package`) with the reason written beside it. | P0 | conversation-2026-09-21 |
