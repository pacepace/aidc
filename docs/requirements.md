...
| MCP-12 | Dev containers MUST NOT be able to reach `aidc-mcp` (separate Docker network, no shared volumes). A separate network is not enough on its own: `aidc-mcp` listens on a host interface, which a session's Squid can reach, so each session's Squid MUST deny the configured `mcp.bind_address`:`mcp.port` (that port on every destination when bound to all interfaces) ahead of its allow rule, and MUST fail to start rather than run without that rule when one is configured. | P0 | plan-2026-05-22; conversation-2026-09-17 |
