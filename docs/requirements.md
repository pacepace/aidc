...
| CTR-13 | The dev container MUST own its Claude Code config directory (`CLAUDE_CONFIG_DIR=/home/vscode/.claude`, on the dev-home volume). The host's `~/.claude/.credentials.json` and `~/.claude.json` MUST NOT be bind-mounted: Claude Code replaces both by rename, so a single-file bind mount goes stale on one side and is unwritable on the other (anthropics/claude-code#18443). Auth is either `CLAUDE_CODE_OAUTH_TOKEN` from `aidc claude-token` (inference-only; no Remote Control) or a `/login` inside the session, which MUST persist across `aidc restart` / `aidc upgrade` and is discarded by `aidc kill`. `aidc create` MAY seed the container's `~/.claude.json` from the host's so first launch skips onboarding; the seed MUST drop the host account, API keys, MCP server definitions and every other project's state, keeping only this project's trust / onboarding / allowed-tools entry. Sharing the whole `~/.claude` directory is rejected: it would expose every project's memory and history to the sandbox, and `~/.claude.json` lives outside it. Per-project memory (`~/.claude/projects/<encoded>/`), `settings.json` and plugins stay bridged from the host as before. `settings.json` MUST NOT be bind-mounted either: it carries `hooks` the HOST's Claude Code runs, so a session that could write it could run a command on the host. It is copied in at create and installed on the session's first start (until 2026-09-22 it was mounted read-write). The status-line script `~/.claude/statusline-command.sh` MAY be bridged read-only when `settings.json` names it, and ONLY that file, as a regular non-symlinked file, since nothing the session could have written may choose a mount source (amended 2026-09-22, review rev-20260922T015403Z-28cf6fcf). | P0 | conversation-2026-09-11 |
| CTR-14 | A session's Claude scratchpad MUST survive moving between host and container. Claude Code keeps per-session working files at `/tmp/claude-<uid>/<encoded-repo>/<session-id>/{scratchpad,tasks}`, which on the container's writable layer are destroyed by `aidc upgrade` and `aidc kill` — so a session that moved kept its transcript (CTR-13) and lost its working files. `aidc create` MUST bind-mount the host's `/tmp/claude-<uid>/<encoded-repo>/` at the identical path inside, keyed on the same encoded repo path as the memory bridge, toggleable via `share_scratchpad` (default true). The mount MUST be the per-repo subdirectory only, never the whole `/tmp/claude-<uid>` root, which holds every other project's scratchpad — the same rejection CTR-13 makes for `~/.claude`. It MUST be suppressed when the host uid is not the container's: the host dirs are mode 0700, so such a mount lands unwritable and Claude cannot create a scratchpad at all. Durability beyond that is explicitly NOT required — `/tmp` is reboot-cleared on the host, so a bridged scratchpad lives exactly as long as a host session's, and `aidc kill` does not reap the host-side directories. Preparing those directories MUST NOT be able to fail a session: both levels sit under world-writable sticky `/tmp`, so a path already held by another user or by a symlink MUST be refused rather than followed, and every failure MUST cost only the bridge. KNOWN GAP: uid equality is a proxy for the real precondition (can the container write the host dir). On macOS, where Docker Desktop remaps bind-mount ownership, it suppresses the bridge on a platform CTR-11 supports; unverified on that platform. | P1 | conversation-2026-09-12 |
...
| NET-06 | A **refresher sidecar** MUST run alongside Squid in the proxy compose stack: fetch threat feeds on startup and every 6h thereafter, atomically write the blocklist (squid follows it without a reload: NET-16; until 2026-09-22 the refresher signalled squid to reload via a shared PID namespace). | P0 | plan-2026-05-21 |
...
| NET-15 | The operator MUST be able to name TCP destinations (`host:port`) a proxied session may reach — in their own config (`egress_tcp:`), with `aidc create --egress-tcp`, or live with `aidc egress <session> add` — without giving the session a route out. Each destination host MUST get one relay on the session network, reachable under the host's real name so TLS hostname checks still pass, forwarding only to that host's resolved address on the listed ports, and logging every connection to the session's audit dir. One relay per host, not per host:port: Docker answers a network alias with every container that carries it, so two relays sharing a hostname could hand a client the relay for the wrong port. The hostname MUST be resolved on the host (whose resolver can see overlay and split-horizon names) when the relay is created. Exception: a session created through aidc-mcp's `session_create` runs `aidc create` inside the aidc-mcp container, which cannot use the host's resolver, so its names are resolved there (the host's upstream DNS, without per-link resolvers); this is documented to operators, who can list an IP address instead. A destination that does not resolve, or that is the aidc-mcp address and port (MCP-12), MUST be refused. Relays MUST survive `aidc restart` and `aidc upgrade` (declared and live alike: they depend on neither the dev container nor its image) and MUST be removed by `aidc kill`. | P1 | conversation-2026-09-21 |
| NET-16 | A new malware blocklist MUST take effect without squid dropping connections: squid never reloads for it (a squid reload is a restart and refused every connection for ~20 s with a multi-million-entry list; issue #34). Squid asks a lookup helper (its `external_acl_type` hook), which reads the byte-sorted list on disk and starts using a replacement the moment the refresher swaps it in. A listed domain MUST block its subdomains too, which is what the feeds intend and what the taint classifier already assumes. The helper MUST fail closed when it has no list to read. | P0 | conversation-2026-09-22 |
...
| CLI-21 | Retired: superseded by CTR-13 (the container owns its Claude config directory; no host credentials are bridged, so there is nothing for a host-side daemon to synchronise). | — | — |
| CLI-22 | Retired: superseded by CTR-13 (the container owns its Claude config directory; no host credentials are bridged, so there is nothing for a host-side daemon to synchronise). | — | — |
| CLI-23 | Retired: superseded by CTR-13 (the container owns its Claude config directory; no host credentials are bridged, so there is nothing for a host-side daemon to synchronise). | — | — |
...
| MCP-08 | The server MUST expose the documented tool surface (session_list/status/exec, session_invoke, session_send and the watch tools, file_get/put, audit_get, taint_mark). `session_create` is exposed only under MCP-38; `session_kill` is deliberately not exposed. | P0 | docs/design-08-mcp-control.md; review 2026-09-17 |
...
| MCP-12 | Dev containers MUST NOT be able to reach `aidc-mcp` (separate Docker network, no shared volumes). A separate network is not enough on its own: `aidc-mcp` listens on a host interface, which a session's Squid can reach, so each session's Squid MUST deny the configured `mcp.bind_address`:`mcp.port` (that port on every destination when bound to all interfaces) ahead of its allow rule, and MUST fail to start rather than run without that rule when one is configured. The rule is the whole enforcement, so it covers a session whose egress goes through that Squid: a session created with `--egress direct` (whose bridge is NATed by design) or attached to another network with `--network` can route around it, and the docs MUST say so where the guarantee is stated. | P0 | plan-2026-05-22; conversation-2026-09-17; review 2026-09-17 |
...
| MCP-24 | A Stop-hook pushback MUST reopen the turn it interrupts. When Claude Code records a blocked stop (an `isMeta` user line starting `Stop hook feedback:`) after a terminal, that terminal MUST NOT be delivered as a completed turn; the turn is delivered once, at the terminal Claude reaches after the pushback. A `stop_hook_summary` with non-empty `hookErrors` MUST NOT by itself be read as a block (it also lists hooks that failed without blocking). An `isMeta` user line MUST NOT close a turn. While a reply's Stop hooks have not reported, delivery SHOULD be held for a bounded time so a slow blocking hook does not split the turn. | P0 | conversation-2026-09-17 |
| MCP-25 | When Claude Code's end-of-turn record (`system` `turn_duration`) follows a turn's terminal, the watcher SHOULD deliver that turn without waiting out the settle window. The record MUST NOT be required: without it, the settle window applies unchanged. | P1 | conversation-2026-09-17 |
| MCP-26 | A turn interrupted at the terminal (the transcript's `[Request interrupted by user` line) MUST be delivered to the orchestrator with `interrupted: true` and `ok: true` (an interrupt is not a failure), its content opening with a note that the person at the terminal interrupted the task and that the text is incomplete, followed by the text Claude wrote before the interrupt (or a statement that it had written none). | P0 | conversation-2026-09-17 |
| MCP-27 | Replies to prompts typed at the terminal MUST keep being delivered: the shared session is a collaboration surface the orchestrator watches. The payload MUST carry `speaker`: `human` when every prompt the turn answers was typed at the terminal, `agent` otherwise. `speaker` MUST NOT be sent until the orchestrator displays `human` turns rather than dropping them; it MUST therefore be controlled by an aidc setting (`metallm.send_speaker`) that is off by default. | P1 | conversation-2026-09-17 |
| MCP-28 | Whether Claude is busy MUST be decided from the transcript, not from pane stability; `_wait_for_idle` MUST be retired from every send path (`session_send`, the queue drainer, `session_run`, `session_resend`). After a prompt is injected, the session MUST count as busy until the transcript records that prompt (as a prompt line, or as a queued message), for a bounded time; a prompt that never shows MUST be logged and MUST NOT be re-sent. Two consecutive free readings MUST be required before a paste. The pane MAY be read only to detect unsent text in Claude's input box, whether Claude is at its input prompt, and, only when the transcript shows an open turn that has gone quiet, whether the status row shows Claude working and whether the input box holds a prompt the transcript shows unanswered (the prompt Claude Code restores after an Esc); an unrecognized screen MUST count as not safe to type. Process liveness (`pane_current_command`) remains. | P0 | conversation-2026-09-17 |
| MCP-29 | A prompt MUST NOT be injected while Claude's input box holds unsent text. While a prompt waits for that reason, the session's tmux status line MUST show that an orchestrator message is waiting and how to let it through. | P0 | conversation-2026-09-17 |
| MCP-30 | A prompt accepted into a session's send queue MUST NOT be dropped. The queue MUST have no wait deadline, MUST hold prompts while Claude is not running, MUST retry failed pastes without an attempt limit, and MUST be persisted so it survives `aidc-mcp` restart. A prompt leaves the queue only by being injected or by its session being killed, in which case it MUST be written to the send dead-letter dir. The orchestrator MUST be able to see why each prompt is waiting (`session_send` result, `session_status`). | P0 | conversation-2026-09-17 |
| MCP-31 | An `aidc-mcp` server started with `AIDC_MCP_ALLOWED_SESSIONS` (a comma- or space-separated list of session names) MUST refuse every tool call and resource read for any other session, MUST refuse `session_create`, MUST list only the allowed sessions, MUST refuse the global config resource, and MUST NOT resume or dead-letter another session's persisted send queue. Unset or empty, nothing is restricted. This is the mode for an MCP whose token is given to an orchestrator running inside a sandbox (the orchestrator's own dev container), where an unscoped token would reach every session on the host. | P1 | conversation-2026-09-17 |
...
| MCP-38 | `session_create` MUST be offered only when the operator turns it on (`mcp.session_create`), because creating a session needs their home mounted into the `aidc-mcp` container; with it off the tool MUST NOT be registered at all rather than registered and failing. When it is on, the server MUST refuse a `repo` or `workspace` outside the directory the operator exposed: those arguments become host bind mounts in the new session, while the server checks them against its own filesystem, so any path present in both (e.g. `/mnt`, `/srv`, `/tmp`) would otherwise mount the host's directory into the session. A scoped server MUST refuse it either way. | P0 | review 2026-09-17 |
...
| SEC-09 | A config file inside the workspace or the repo (`.aidc/config.yaml`) is writable from inside the session it configures, so it MUST NOT be able to widen the sandbox. From there only settings that cannot open a path out or reach the host apply: `profile`, `claude_mode`, `claude_resume`, additions to `state_actor_tlds`, `blocklist_additions` and `container_only_paths`, and a change that only tightens (`egress: proxied`, `share_*: false`, `tld_taints: true`, a `taint_response` at least as strict as the current one). Everything else it sets — `egress: direct`, `networks`, `ports`, `egress_tcp`, `dns_servers`, `audit_dir`, `notify_webhook`, `share_*: true`, a softer `taint_response` — MUST be reported by `aidc create` and NOT applied unless the operator passes `--trust-repo-config`. The operator's own config and CLI flags are trusted. | P0 | conversation-2026-09-21 |
...
| REL-09 | Supply-chain cooldown: a dependency version MUST NOT be adopted until it has been published for at least 14 days, so a hijacked or malicious release has time to be found and pulled before aidc installs it. Each package manager's own setting enforces it: for the MCP server, `exclude-newer = "14 days"` in `mcp/pyproject.toml` (recorded in `uv.lock`); in the dev image, system defaults for uv (`/etc/uv/uv.toml`), pip (`/etc/pip.conf`, `uploaded-prior-to = P14D`) and npm (`/etc/npmrc`, `min-release-age=14`), which also cover packages installed inside a session while leaving a project's own config in charge of itself. A unit test pins all of them. Exempt: Claude Code (kept current, Pace 2026-09-21), the Go/Rust/CPython toolchains (straight from their makers) and apt packages (holding back distro security updates does more harm). A fix that must land sooner is an explicit exception for that one package, with the reason written beside it; inside a session, one command can override (`uv --exclude-newer false`, `pip --uploaded-prior-to <now>`, `npm --min-release-age=0`). | P0 | conversation-2026-09-21 |
