...
| MCP-24 | A Stop-hook pushback MUST reopen the turn it interrupts. When Claude Code records a blocked stop (an `isMeta` user line starting `Stop hook feedback:`, or a `system` `stop_hook_summary` line with non-empty `hookErrors`) after a terminal, that terminal MUST NOT be delivered as a completed turn; the turn is delivered once, at the terminal Claude reaches after the pushback. An `isMeta` user line MUST NOT close a turn. | P0 | conversation-2026-09-17 |
