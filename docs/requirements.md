...
| NET-15 | The operator MUST be able to name TCP destinations (`host:port`) a proxied session may reach — in their own config (`egress_tcp:`), with `aidc create --egress-tcp`, or live with `aidc egress <session> add` — without giving the session a route out. Each destination MUST get its own relay on the session network, reachable under the destination's real hostname so TLS hostname checks still pass, forwarding only to that one address and port, and logging every connection to the session's audit dir. The hostname MUST be resolved on the host (whose resolver can see overlay and split-horizon names) when the relay is created. A destination that does not resolve, or that is the aidc-mcp address and port (MCP-12), MUST be refused. Relays MUST survive `aidc restart` and `aidc upgrade` (declared and live alike: they depend on neither the dev container nor its image) and MUST be removed by `aidc kill`. | P1 | conversation-2026-09-21 |
