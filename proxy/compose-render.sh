...
# TCP egress relays (NET-15), rendered by aidc create from lib/egress.sh.
...
# aidc-mcp's bind address and port ("addr:port"), which the squid entrypoint
# denies to the dev container (MCP-12). Empty only for a compose file rendered
# outside `aidc create`.
: "${MCP_DENY:=}"
export MCP_DENY
