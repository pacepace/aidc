...
#   2. adds the PUBLIC half to pacepace/homebrew-aidc as a write deploy key,
#   3. stores the PRIVATE half as the TAP_DEPLOY_KEY Actions secret on
#      pacepace/aidc (the name release.yml reads),
#   4. only then removes any previous deploy key of the same title (rotation),
#      so a failure part-way leaves the old key working rather than none,
...
installed_keys() {
    gh api "repos/${TAP_REPO}/keys" \
        --jq ".[] | select(.title == \"${KEY_TITLE}\") | \"\(.id) \(.key)\"" \
...
status() {
    ...
    if gh secret list -R "$SRC_REPO" | awk '{print $1}' | grep -qx "$SECRET_NAME"; then
...
# Capture what is live now; it is removed only after the new key is fully in
# place, so a failure in between leaves the old key working rather than none.
previous="$(installed_keys)"
...
# Rotate: drop the previous key(s) so exactly one is live.
printf '%s\n' "$previous" | while read -r id old_fp; do
    [ -n "$id" ] || continue
